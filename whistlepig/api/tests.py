...
class ApiTest(TestCase):
    ...
    def test2_get_api_statusupdate(self):
        response = self.client.get('/api/v1/statusupdate/1/', follow=True)
        ...
        self.assertEqual(response_obj['posted_by'],'Matthew Zeier')
        self.assertEqual(response_obj['admin_assigned'],'Matthew Zeier')

    def test3_get_api_statusupdate_with_service_outages(self):
        response = self.client.get('/api/v1/statusupdate/9/', follow=True)
        response_obj = json.loads(response.content)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response_obj['from_bugzilla'],False)
        ...
        self.assertEqual(response_obj['admin_assigned'],'cshields')
        self.assertEqual(len(response_obj['service_outages_set']),2)
        self.assertEqual(response_obj['service_outages_set'][0],'/api/v1/serviceoutage/4/')
        self.assertEqual(response_obj['service_outages_set'][1],'/api/v1/serviceoutage/5/')
